...
public class AdvancedCellposeDetectorModel extends DetectorSweepModel
{
	...
	public AdvancedCellposeDetectorModel()
	{
		super( "Cellpose advanced detector", createModels(), createFactory() );
	}

	private static SpotDetectorFactoryBase< ? > createFactory()
	{
		if ( null == new DetectorProvider().getFactory( "CELLPOSE_ADVANCED_DETECTOR" ) )
			return null;
		else
			return AdvancedCellposeOpt.createFactory();
	}
	...
	private static Map< String, AbstractParamSweepModel< ? > > createModels()
	{
		...
		{
			return AdvancedCellposeOpt.createModels();
		}
	}
}
