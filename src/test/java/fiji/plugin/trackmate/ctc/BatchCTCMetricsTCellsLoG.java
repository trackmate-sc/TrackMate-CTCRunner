 ...
 *
 */
public class BatchCTCMetricsTCellsLoG
{
	...
	public static void main( final String[] args ) throws IOException
	{
		...
		final String groundTruthPath = new File( rootFolder, "01_GT" ).getAbsolutePath();
		try (final Context context = new Context())
		{
			...
			// Configure detection.
			final SpotDetectorFactoryBase< ? > detectorFactory = new LogDetectorFactory<>();
			final Map< String, Object > detectorSettings = detectorFactory.getDefaultSettings();

			// Sweep over radius and quality, coupled.
			final double[] logRadiuses = new double[] { 4., 4.5, 5. };
			final double[] logThresholds = new double[] { 9., 7., 6.5 };

			// Loop over log parameters.
			for ( int i = 0; i < logThresholds.length; i++ )
			{

				detectorSettings.put( DetectorKeys.KEY_RADIUS, logRadiuses[ i ] );
				detectorSettings.put( DetectorKeys.KEY_THRESHOLD, logThresholds[ i ] );

				// Exec detection.
				final ValuePair< TrackMate, Double > detectionResult = runner.execDetection( detectorFactory, detectorSettings );
				...
				for ( final SpotTrackerFactory trackerFactory : trackerFactories )
				{
					for ( final int frameGap : maxFrameGaps )
					{
						for ( final double mld : maxLinkingDistances )
						{
							...
							{
								System.err.println( "Inconsistency in the CTC export." );
								System.err.println( "Saving TrackMate file for inspection." );
								final File trackmateFile = new File( rootFolder, "TrackMate-CTCCrash.xml" );
								final TmXmlWriter writer = new TmXmlWriter( trackmateFile, Logger.DEFAULT_LOGGER );
								...
								{
									writer.writeToFile();
									System.err.println( "Data saved to: " + trackmateFile.toString() );
								}
